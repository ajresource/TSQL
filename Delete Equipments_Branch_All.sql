...
UPDATE AMT_TYPED_VARIABLE SET Varchar_Value = @MaxPlanningWOtoCreate WHERE Value_Name ='MaxPlanningWOtoCreate'
	...
	DELETE FROM dbo.tblProjBillings PB INNER JOIN
	tblEqpProjs EP ON PB.EqpProjId=EP.EqpProjId 
	WHERE EP.EqpPlanId NOT IN(
	SELECT EqpPlanId FROM dbo.EQUIPMENT_HIERARCHY_V eqv WHERE 
	BranchId=24)
	
	

	DELETE FROM dbo.tblProjBillingAmounts PBA INNER JOIN
