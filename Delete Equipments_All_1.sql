...
where Site='Chimiwungo-Unknown - Terminated'
...
BEGIN
		...
		BEGIN
		UPDATE TASK Set Work_Order_ID=NULL where Eqp_Plan_ID= @EquipmentId
		...
		DELETE FROM PH_PLANT_HIRE_RATE_DETAIL WHERE Plant_Hire_Rate_Id IN 
		(SELECT Plant_Hire_Rate_Id from PH_PLANT_HIRE_RATE WHERE Eqp_Plant_Hire_Id IN 
		(SELECT Eqp_Plant_Hire_Id FROM PH_EQP_PLANT_HIRE WHere Eqp_Plan_id = @EquipmentId))

		Delete from PH_PROJECT_CHARGE_SIMULATOR WHERE Plant_Hire_Rate_Id IN 
		(SELECT Plant_Hire_Rate_Id from PH_PLANT_HIRE_RATE WHERE Eqp_Plant_Hire_Id IN 
		(SELECT Eqp_Plant_Hire_Id FROM PH_EQP_PLANT_HIRE WHere Eqp_Plan_id = @EquipmentId))
