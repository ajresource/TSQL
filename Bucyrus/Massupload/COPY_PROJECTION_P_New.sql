/****** Object:  StoredProcedure [dbo].[COPY_PROJECTION_P]    Script Date: 01/29/2013 11:54:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[COPY_PROJECTION_P]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[COPY_PROJECTION_P]
GO
...
CREATE PROCEDURE [dbo].[COPY_PROJECTION_P]  
/******************************************************************************  
...
SELECT  EQ.EqpplanID,EQ.EqpprojID,EQ.EqPplan,EQ.Projection_Type_ID,EQ.Projection_Type_Name,PJ.ProjTaskId,PJ.Task_Header_ID,
...
-- UPDATE PARTS , IF CopyPart=1 primary parts will get copied from the old projections, CopyPart=0 copy the parts from the new Centreline
update PJ
Set PJ.Part_ID=NULL
from tblProjtasks PJ
...
BEGIN
update PT 
set PT.Part_ID=ZZ.Part_Id
from ZZ_OLD_PROJECTION ZZ
inner join tblProjtasks PT ON ZZ.Task_Header_ID=PT.Task_Header_ID AND ZZ.EqpprojID=PT.EqpprojID
END
ELSE
BEGIN
update PJ
...
-- COPY FREQUENCYF CopyFrequency=1 Frequency will get copied from the old projections, CopyFrequency=0 copy the Frequency from the new Centreline

IF @CopyFrequency=1
BEGIN
update PT
Set PT.frequency=ZZ.frequency
from tblProjtasks PT
INNER JOIN ZZ_OLD_PROJECTION ZZ ON ZZ.Task_Header_ID=PT.Task_Header_ID AND ZZ.EqpprojID=PT.EqpprojID

update PTO
Set PTO.frequency=ZZ.frequency
--select PTO.frequency,ZZ.frequency
