...
/****** Object:  View [dbo].[mm_ITOPS_CurrentTimeline]    Script Date: 07/02/2013 10:16:32 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[mm_ITOPS_CurrentTimeline]
AS


SELECT * ,  1 as allday, 
'Ontime ID: ' + Cast(TaskId as varchar) + ' - ' + Name + ' - BH: ' + CAST([Budgeted Hours]as varchar) + ' - AH: ' + CAST([Actual Hours Hours] as varchar) + ' - TC: ' + iCode as [Appointment Desc],  
DATEADD(DAY,-[Budgeted Hours]/8,Convert(Datetime,Required_Date, 102)) as BudgetStartDate ,
CASE WHEN StepName = 'Completed IT Task' and ([Actual Hours Hours] = 0 OR [Budgeted Hours] = 0) THEN 100 ELSE
[Actual Hours Hours]/[Budgeted Hours]*100 END AS PercentComplete,
CASE
